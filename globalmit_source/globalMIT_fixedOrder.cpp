...
//     High order DBN, parallel, fixed order version	//
//     Author          : Vinh Nguyen                    //
//							//
...
namespace globalMIT_fixedOrder {
#endif
...
int main(int argc, char* argv[]) {
  ...
  outLog<<endl<<"---GlobalMIT for High Order DBN structure learning (fixed order version)---"<<endl;
  cout  <<endl<<"---GlobalMIT for High Order DBN structure learning (fixed order version)---"<<endl;
  ...
  */
  ...
  for(int i=i_start;i<=i_end;i++){//loop through the set of selected variables
    ...
    double * score=new double[dim]; //1-d array to store the scores of all parent combination
    double * new_score=NULL; //1-d array to store the scores of all parent combination
    int* ca=NULL;
		
    int* cb=new int[1];
    //find the best lag for each node
    int* lag=new int[dim];
    for(int nodeID=0;nodeID<dim;nodeID++){
      double best_MI=0;
      int best_lag=1;
      if(d>1){
	for(int l=1;l<=d;l++){
    ...
    for(int p=1;p<Pstar;p++) { //loop through parent sets of increasing size
      ...
      ca=new int[dim];//set of all parents
      cb=new int[p];  //subset of parent of cardinality p
      for(int j=0;j<dim;j++)     {ca[j]=j;}
      for(int j=0;j<p;j++)	   {cb[j]=j;}
      //cout<<"P="<<p<<" Allocating "<<nchoosek(dim,p)<< "*"<< sizeof(double)<<" bytes = "<< nchoosek(dim,p)*sizeof(double) <<" bytes =" << nchoosek(dim,p)*sizeof(double)/(1024*1024)<<" Mb of memory"<<endl;
      ...
      do  //generate all parent combination and score
	{
	  ...
	  if (p==1){ //only canculate the score for the 1st level
	    double CMI=conditional_MI_DBN_DBN_HighOrder_ab(data,cb[0]+(lag[cb[0]]-1)*dim, i, 0,cb,d,n_state);
	    ...
	    int *cb_1=new int[p];for(int j=0;j<p;j++){cb_1[j]=cb[j]+(lag[cb[j]]-1)*dim;}
	    int pos=findLexicalIndex(dim,p-1,cb);
	    score_i=score[pos];
					 
	    //calculate the last score and store
	    double CMI=conditional_MI_DBN_DBN_HighOrder_ab(data,cb_1[p-1],i, p-1 ,cb_1,d,n_state);
	    score_i+=CMI;	
    ...
    for(int k=0;k<best_nPa;k++) {
      ...
      net_order[best_Pa[k]%dim][i]= lag[best_Pa[k]%dim];
    }
  ...
  double best_score=0;
  for(int i=i_start;i<=i_end;i++) best_score+=2*(Ne)*HX_arr[i]-best_score_arr[i];
...
double getScore(int **net){
  ...
  for(int i=0;i<dim;i++){
    ...
    for(int j=0;j<nPa;j++){
      score_i+=2*(N-d)*conditional_MI_DBN_DBN_HighOrder(data, Pa[j],i,j,Pa,d,n_state)-chi[j+1];
    }
...
extern "C" {
  ...
  int globalMIT_fixedOrder_exe_wrapper(array2d* input_data, int n, int Dim, int n_segs, int lens[],
     ...
     */
    using namespace globalMIT_fixedOrder;
